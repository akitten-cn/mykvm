...
|T04.b|进行中|T04.b1 已把认证连接、V2 会话 gate 和 InjectorPort 接通并以 FakeInjector 验证；T04.b2 生产 AppRuntime/原生端口启动接线待实现|
...
下一原子任务为 T04.b2，把已验证的接收会话适配器接入 AppRuntime 和普通用户原生注入端口。旧 LAN 开关仍保持关闭。可靠释放账本仍属于 T07；热键、剪贴板、完整中文设置、后台生命周期和预览打包仍需按任务表实现与验收。
